        ...
        RF_EventMask aRfEventMask)
{
	...
	struct ieee802154_cc13xx_cc26xx_data *drv_data = dev->data;
        ...
        if (drv_data->cmd_ieee_ed_scan.status != IEEE_DONE_OK) {
                printk("ED Scan failed: %x\n", drv_data->cmd_ieee_ed_scan.status);
                drv_data->ed_scan_done_cb(dev, IEEE802154_MAC_RSSI_DBM_UNDEFINED);
                return;
        }

        drv_data->ed_scan_done_cb(dev, drv_data->cmd_ieee_ed_scan.maxRssi);
}
                                     ...
                                     energy_scan_done_cb_t done_cb)
{
	...
	struct ieee802154_cc13xx_cc26xx_data *drv_data = dev->data;
        ...
        drv_data->cmd_ieee_ed_scan.endTime = duration * (CC13XX_CC26XX_RAT_CYCLES_PER_SECOND / 1000);
	...
	k_mutex_lock(&drv_data->tx_mutex, K_FOREVER);
        ...
        reason = RF_runScheduleCmd(drv_data->rf_handle,
                ...
                rfCoreEdScanCmdCallback, RF_EventLastCmdDone);
